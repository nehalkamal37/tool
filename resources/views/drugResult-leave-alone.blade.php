            ...
            @foreach ($drug_data as $item)
                <tr>
                    ...
                    <td>
                        ...
                        <a style="color: black" href="https://ndclist.com/ndc/{{ substr($item->ndc, 0, strrpos($item->ndc, '-')) }}/package/{{ $item->NDC }}" 
                            target="_blank">
                             {{ $item->ndc }}
                         </a>
...
<script>
    $(document).ready(function () {
        $('.ndc-link').on('click', function (e) {
            e.preventDefault(); // Prevent default link behavior

            const mainNdc = $(this).data('main-ndc');
            const packageNdc = $(this).data('package-ndc');
            const url = `https://ndclist.com/ndc/${mainNdc}/package/${packageNdc}`;
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    // Example: Display fetched data in a modal
                    $('#ndcModal .modal-body').html(response);
                    $('#ndcModal').modal('show');
                },
                error: function (error) {
                    console.error('Error fetching NDC data:', error);
                    alert('Failed to fetch NDC data. Please try again.');
                }
            });
        });
    });
